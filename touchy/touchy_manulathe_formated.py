#! /usr/bin/python2
# Touchy is Copyright (c) 2009 Chris Radek 
#
...
class touchy:
    def __init__(self, inifile):
        ...
        stats = [
            ...
            'onlimit', 'spindledir', 'spindlespeed', 'loadedtool', 'preppedtool',
            'xyrotation', 'tlo', 'activecodes', 'spindlespeed2', 'label_g5xoffset',
        ...
        self.status = emc_interface.emc_status(
            gtk, linuxcnc, self.listing, self.hal,
            ...
            stats, floods, mists, spindles,
            prefs, opstop, blockdel, spindle_values
        )
    ...
    def send_message(self, socket, dest_xid, message):
        ...
        event.data = message  # must be exactly 20 char bytes
        event.send_event = True  # signals this was sent explicitly
    ...
    def toolset_fixture(self, b):
        if self.radiobutton_mask:
            return self.prefs.putpref('toolsetting_fixture', 1)
        self.g10l11 = 1

    def toolset_workpiece(self, b):
        if self.radiobutton_mask:
            return self.prefs.putpref('toolsetting_fixture', 0)
        self.g10l11 = 0
    ...
    def setfont(self):
        ...
        for i in [
            ...
            "toolset_workpiece", "toolset_fixture", "change_theme", "reset_spinde_index",
            "shut_down"
        ]:
        ...
        # labels for mdi and filechooser
        for i in range(self.num_mdi_labels):
        ...
        for i in [
            "mdi", "startup", "manual", "auto", "preferences", "status",
    ...
    def periodic_radiobuttons(self):
        # Placeholder for periodic radiobutton updates if needed.
        return True

    def fullscreen_startup(self):
        if not self.fullscreen_startup_processed:
            if self.fullscreen:
                self.wTree.get_widget('MainWindow').fullscreen()
            self.fullscreen_startup_processed = 1
        return False

    def hack_leave(self, b):
        # Placeholder for any required post-release actions.
        pass
    ...
    def set_dynamic_tabs(self):
        # Placeholder: initialize dynamic tabs if needed.
        pass

    def save_maxvel_pref(self):
        self.prefs.putpref('maxvel', self.mv_val, float)
...
if __name__ == "__main__":
    ...
    app = touchy(inifile)
    # parse sys.argv to store non-default axis to joint assignments
    # interpret coords in trivkins
    res = app.trivkins()

    # if no second arg => no postgui line is present in the ini file
    postgui_halfile = None
    if len(sys.argv) > 2:
        postgui_halfile, sys.argv[2] = app.postgui()

    if postgui_halfile:
        # halcmd -I -f postgui_halfile
        from hal_interface import halfile_loader
        h = halfile_loader().cmdswitch("-I", "halcmd", ["-f", postgui_halfile])
        gtk.main()
    else:
        gtk.main()
